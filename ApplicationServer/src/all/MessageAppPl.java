...
public class MessageAppPl {
	...
	public int type;	// receive    =0:手番交代時  =1:石を置く  =2:カード使用  =3:時間切れ通知
						// send       =0:手番交代時  =1:現在の盤面を返す =2:カード使用 =3:時間切れ通知 =4:ゲーム終了時
	...
	public Stone[][] board = new Stone[8][8];		// send		=0:盤面の状態（白黒)
	public int[][] cellRate;	// 
	...
	MessageAppPl() {
		...
		white = new PlayerInfo("1", Color.None);
		white = new PlayerInfo("2", Color.None);
	}
}
